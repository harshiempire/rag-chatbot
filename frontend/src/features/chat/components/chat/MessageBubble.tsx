            ...
            {showSources && (
              <div className="mt-2 grid gap-2 grid-cols-1 sm:grid-cols-2">
                {message.sources?.map((source) => (
                  <div
                    key={source.id}
                    className="bg-slate-800/50 border border-slate-700 p-3 rounded-lg text-xs hover:bg-slate-800 transition-colors cursor-pointer group"
                  >
                    <div className="font-semibold text-slate-300 mb-1 truncate group-hover:text-blue-400 transition-colors">
                      {source.title}
                    </div>
